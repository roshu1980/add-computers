...
class Cmsmart_Dailydeal_Block_Upcomingdeal extends Mage_Catalog_Block_Product_Abstract
{
	public $configP = '';
	...
	public function __construct()
	{
		...
		// echo 'e.entity_id = t2.productid '.($from?' and t2.starttime >= "'.$datefrom.'"':' and t2.starttime >= "'.$todayDate.'"').($to?' and t2.starttime <= "'.$dateto.'"':'').')';
		$collection->getSelect()->join(array('t2' => Mage::getSingleton('core/resource')->getTableName('dailydeal/dailydealproducts')),'(e.entity_id = t2.productid and t2.status != 2 '.($from?' and t2.starttime >= "'.$datefrom.'"':' and t2.starttime >= "'.$todayDate.'"').($to?' and t2.starttime <= "'.$dateto.'"':'').')', array('t2.sold', 't2.save', 't2.quantity', 't2.starttime', 't2.closetime'));
