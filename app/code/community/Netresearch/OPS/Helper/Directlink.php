    ...
    public function processFeedback($order, $params)
    {
        ...
        if (false == $this->isValidOpsRequest($transaction, $order, $params)) {
            $order->addStatusHistoryComment(
                Mage::helper('ops')->__(
                    'Could not perform actions for Barclaycard status: %s.',
                    Mage::helper('ops')->getStatusText($params['STATUS'])
                )
            )->save();
            throw new Mage_Core_Exception('invalid Barclaycard request');
        }
        ...
        switch ($params['STATUS']) {
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_REFUND_UNCERTAIN_STATUS :
                $order->addStatusHistoryComment(
                    Mage::helper('ops')->__(
                        'Refund is waiting or uncertain. Barclaycard status: %s.',
                        Mage::helper('ops')->getStatusText($params['STATUS'])
                    )
                );
                $order->save();
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_REFUND_DECLINED_ACQUIRER :
                $this->closePaymentTransaction(
                    ...
                    Mage::helper('ops')->__(
                        'Refund was refused. Automatic creation failed. Barclaycard status: %s.',
                        Mage::helper('ops')->getStatusText($params['STATUS'])
                    )
                );
                break;
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_PAYMENT_IN_PROGRESS:
                $order->addStatusHistoryComment(
                    Mage::helper('ops')->__(
                        'Capture is waiting or uncertain. Barclaycard status: %s.',
                        Mage::helper('ops')->getStatusText($params['STATUS'])
                    )
                );
                $order->save();
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_AUTH_REFUSED :
                $this->closePaymentTransaction(
                    ...
                    Mage::helper('ops')->__(
                        'Capture was refused. Automatic creation failed. Barclaycard status: %s.',
                        Mage::helper('ops')->getStatusText($params['STATUS'])
                    )
                );
                break;
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_VOID_UNCERTAIN:
                $order->addStatusHistoryComment(
                    Mage::helper('ops')->__(
                        'Void is waiting or uncertain. Barclaycard status: %s.',
                        Mage::helper('ops')->getStatusText($params['STATUS'])
                    )
                );
                $order->save();
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_VOID_REFUSED:
                $this->closePaymentTransaction(
                    ...
                    Mage::helper('ops')->__(
                        'Void was refused. Automatic creation failed. Barclaycard status: %s.',
                        Mage::helper('ops')->getStatusText($params['STATUS'])
                    )
                );
                break;
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_AUTHORIZED_KWIXO:
                ...
                && $order->getPayment()->getMethodInstance() instanceof Netresearch_OPS_Model_Payment_Kwixo_Abstract) {
                    ...
                    $comment = Mage::helper('ops')->__('Authorization status changed. Current Barclaycard status is: %s.', Mage::helper('ops')->getStatusText($params['STATUS']));
//                    $this->closePaymentTransaction($order, null, 'authorization', "", false);
                } else {
                    $order->addStatusHistoryComment(Mage::helper('ops')->__('Authorization status changed. Current Barclaycard status is: %s.', Mage::helper('ops')->getStatusText($params['STATUS'])));
                    $order->save();
                }
                break;
            default:
                $order->addStatusHistoryComment(
                    Mage::helper('ops')->__('Unknown Barclaycard status: %s.', Mage::helper('ops')->getStatusText($params['STATUS']))
                );
                $order->save();
    ...
    public function isValidOpsRequest($openTransaction, Mage_Sales_Model_Order $order, $opsRequestParams)
    {
        ...
        if ($this->getTypeForStatus($opsRequestParams['STATUS']) == Netresearch_OPS_Model_Payment_Abstract::OPS_CAPTURE_TRANSACTION_TYPE) {
            if (is_null($requestedAmount)) {
                Mage::helper('ops')->log('Please configure Barclaycard to submit amount');
                return false;
            }
    ...
    public function performDirectLinkRequest($quote, $params, $storeId = null)
    {
        ...
        if (!is_null($response['STATUS'])
            ...
            throw new Mage_Core_Exception(Mage::helper('ops/data')->__('Barclaycard Payment failed'));
        }
        return $response;
    }
}
