...
class Netresearch_OPS_Helper_Payment extends Mage_Core_Helper_Abstract
{
    ...
    private function compareHashes($hashFromOPS, $actual)
    {
        ...
        $helper->log(
            $helper->__(
                "Checking hashes\nHashed String by Magento: %s\nHashed String by Barclaycard: %s",
                $actual,
                $hashFromOPS
            )
        );
        ...
        switch ($params['STATUS']) {
            ...
            case Netresearch_OPS_Model_Payment_Abstract::OPS_PAYMENT_CANCELED_BY_CUSTOMER:
                $this->cancelOrder($order, $params, Mage_Sales_Model_Order::STATE_CANCELED,
                                   Mage::helper('ops')->__(
                                       'Order canceled on Barclaycard side. Status: %s, Payment ID: %s.',
    ...
    /**
     * add fraud detection of Barclaycard to additional payment data
     *
    ...
    public function declineOrder($order, $params)
    {
        try {
            ...
            $order->setState(
                ...
                Mage_Sales_Model_Order::STATE_CANCELED,
                Mage::helper('ops')->__(
                    'Order declined on ops side. Barclaycard status: %s, Payment ID: %s.',
                    Mage::helper('ops')->getStatusText($params['STATUS']),
                    $params['PAYID']
                )
            );
    ...
    public function waitOrder($order, $params)
    {
        try {
            $order->setState(
                ...
                Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW,
                Mage::helper('ops')->__(
                    'Order is waiting for Barclaycard confirmation of 3D-Secure. Barclaycard status: %s, Payment ID: %s.',
                    Mage::helper('ops')->getStatusText($params['STATUS']),
                    $params['PAYID']
                )
            );
            ...
            $this->_getCheckout()->addError(
                Mage::helper('ops')->__('Error during 3D-Secure processing of Barclaycard. Error: %s', $e->getMessage())
            );
            throw $e;
        }
    }
        ...
        switch ($ops_status) {
            case Netresearch_OPS_Model_Payment_Abstract::OPS_PAYMENT_UNCERTAIN :
                $exceptionMessage = Mage::helper('ops')->__(
                    'A technical problem arose during payment process, giving unpredictable result. Barclaycard status: %s.',
                    Mage::helper('ops')->getStatusText($ops_status)
                );
                break;
            default:
                $exceptionMessage = Mage::helper('ops')->__(
                    'An unknown exception was thrown in the payment process. Barclaycard status: %s.',
                    Mage::helper('ops')->getStatusText($ops_status)
                );
        }
    ...
    protected function _processDirectSale($order, $params, $instantCapture = 0)
    {
        ...
        if ($status == Netresearch_OPS_Model_Payment_Abstract::OPS_AWAIT_CUSTOMER_PAYMENT) {
            ...
            $order->setState(
                Mage_Sales_Model_Order::STATE_PROCESSING,
                Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                Mage::helper('ops')->__('Authorization waiting from Barclaycard')
            );
            $order->save();
            ...
            if ($status == Netresearch_OPS_Model_Payment_Abstract::OPS_AUTHORIZED) {
                if ($order->getStatus() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                    $order->setState(
                        Mage_Sales_Model_Order::STATE_PROCESSING,
                        Mage_Sales_Model_Order::STATE_PROCESSING,
                        Mage::helper('ops')->__('Processed by Barclaycard')
                    );
    ...
    protected function _processAuthorize($order, $params)
    {
        ...
        if ($status == Netresearch_OPS_Model_Payment_Abstract::OPS_AWAIT_CUSTOMER_PAYMENT) {
            $order->setState(
                ...
                Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                Mage::helper('ops')->__(
                    'Waiting for payment. Barclaycard status: %s.', Mage::helper('ops')->getStatusText($status)
                )
            );
            ...
            $order->setState(
                ...
                Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                Mage::helper('ops')->__(
                    'Authorization uncertain. Barclaycard status: %s.', Mage::helper('ops')->getStatusText($status)
                )
            );
        } else {
            ...
            $order->setState(
                ...
                Mage_Sales_Model_Order::STATE_PROCESSING,
                Mage::helper('ops')->__(
                    'Processed by Barclaycard. Payment ID: %s. Barclaycard status: %s.', $payId,
    ...
    public function handleUnknownStatus($order)
    {
        if ($order instanceof Mage_Sales_Model_Order) {
            $message = Mage::helper('ops')->__(
                'Unknown Barclaycard state for this order. Please check Barclaycard backend for this order.'
            );
