...
class Netresearch_OPS_Model_Api_DirectLink extends Mage_Core_Model_Abstract
{
     ...
     */
     public function call($params, $url)
     {
         try {
            ...
            Mage::logException($e);
            Mage::throwException(
                Mage::helper('ops')->__('Barclaycard server is temporarily not available, please try again later.')
            );
        }
     }
     ...
     */
     public function performRequest($requestParams, $url, $storeId = 0)
     {
        ...
        $helper->log($helper->__("Direct Link Request/Response in Barclaycard \n\nRequest: %s\nResponse: %s\nMagento-URL: %s\nAPI-URL: %s",
            serialize($params),
      ...
      * @return array | null - null if requests were not successful, array containing Barclaycard payment data otherwise
      * 
      */
     protected function getResponseParams($params, $url, $retryCount = 0)
     {
         ...
         if ($retryCount < self::MAX_RETRY_COUNT) {
            try {
             ...
             Mage::throwException(Mage::helper('ops')->__('An error occured during the Barclaycard request. Your action could not be executed.'));
         }
         return $responseParams;
     }
     ...
     */
     public function checkResponse($responseParams)
     {
         if (false === is_array($responseParams) || false === array_key_exists('NCERROR', $responseParams) || $responseParams['NCERROR'] > 0) {
            ...
            Mage::throwException(
                Mage::helper('ops')->__('An error occured during the Barclaycard request. Your action could not be executed. Message: "%s".',$responseParams['NCERRORPLUS'])
            );
         }
     }
}
