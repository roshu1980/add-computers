...
//        return str_replace('ops_', '', $this->_code);
    ...
    public function getFormFields($order, $requestParams, $isRequest = true)
    {
        ...
        if($isRequest){
            ...
            $helper->log($helper->__("Register Order %s in Barclaycard \n\nAll form fields: %s\nBarclaycard String to hash: %s\nHash: %s",
    ...
    public function _getOrderDescription($order)
    {
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            // we know that Barclaycard is not able to handle characters that are not available in iso-8859-1
//            $descriptionItems[] = mb_ereg_replace("[^a-zA-Z0-9äáàéèíóöõúüûÄÁÀÉÍÓÖÕÚÜÛ_ ]" , "" , $item->getName());
        ...
        try {
            ...
            if ($response['STATUS'] == self::OPS_PAYMENT_PROCESSING ||
                ...
                Mage::helper('ops/directlink')->directLinkTransact(
                    ...
                    $arrInfo,
                    self::OPS_CAPTURE_TRANSACTION_TYPE,
                    $this->getHelper()->__('Start Barclaycard %s capture request',$arrInfo['type']));
                $order = Mage::getModel('sales/order')->load($orderId); //Reload order to avoid wrong status
                $order->addStatusHistoryComment(
                    Mage::helper('ops')->__(
                        'Invoice will be created automatically as soon as Barclaycard sends an acknowledgement. Barclaycard status: %s.',
            ...
            elseif ($response['STATUS'] == self::OPS_PAYMENT_PROCESSED_MERCHANT || $response['STATUS'] == self::OPS_PAYMENT_REQUESTED) {
                 ...
                 Mage::throwException(
                     $this->getHelper()->__(
                         'The Invoice was not created. Barclaycard status: %s.',
                         Mage::helper('ops')->getStatusText($response['STATUS'])
                     )
                 );
            }
    ...
    public function canRefund()
    {
        try {
            ...
            // if Barclaycard transaction is closed then no online refund is possible
            if ($order->getPayment()
            ...
            if (false === Mage::helper('ops/directlink')->hasPaymentTransactions($order,self::OPS_REFUND_TRANSACTION_TYPE)) {
                ...
                Mage::getSingleton('core/session')->addNotice(
                    $this->getHelper()->__('There is already one creditmemo in the queue. The Creditmemo will be created automatically as soon as Barclaycard sends an acknowledgement.')
                );
                $this->getHelper()->redirect(
    ...
    public function cancel(Varien_Object $payment)
    {
        ...
        if (true === $this->canCancelManually($payment->getOrder())) {
            $payment->getOrder()->addStatusHistoryComment(
                $this->getHelper()->__("The order was cancelled manually. The Barclaycard-state is 0 or null.")
            );
            return parent::cancel($payment);
        }
        ...
        try {
            ...
            if ($response['STATUS'] == self::OPS_VOID_WAITING
                ...
                Mage::helper('ops/directlink')->directLinkTransact(
                    ...
                    Mage::helper('ops')->__(
                        'Start Barclaycard void request. Barclaycard status: %s.',
                ...
                Mage::helper('ops/directlink')->directLinkTransact(
                    ...
                    self::OPS_VOID_TRANSACTION_TYPE,
                    $this->getHelper()->__(
                        'Void order succeed. Barclaycard status: %s.',
                        $response['STATUS']
                    ),
                    1
                );
                ...
                Mage::throwException(
                    $this->getHelper()->__(
                        'Void order failed. Barclaycard status: %s.',
                        $response['STATUS']
                    )
                );
            }
     ...
     * users are asked to correct the values before redirect to Barclaycard
     *
     ...
     * @param array                  $formFields    Parameters to be sent to Barclaycard
     * @return bool
     */
    ...
    /**
     * If cart Item information has to be transmittet to Barclaycard
     *
     * @return bool
     */
    ...
    /**
     * Returns array with the order item data formatted in Barclaycard fashion if payment method implementation
