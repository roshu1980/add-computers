    ...
    public function testCanRefund()
    {
        ...
        $this->assertEquals(
            Mage::helper('ops/data')->__(
                'There is already one creditmemo in the queue. The Creditmemo will be created automatically as soon as Barclaycard sends an acknowledgement.'
            ), $message->getText()
        );
...
//            ->with('template')
    ...
    public function testVoidFailsWhenStatusIsUnknown()
    {
        ...
        try {
            ...
            $this->assertEquals($helper->__('Void order failed. Barclaycard status: %s.', 666), $e->getMessage());
        }
    }
