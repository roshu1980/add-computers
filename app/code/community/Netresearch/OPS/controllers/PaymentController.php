...
class Netresearch_OPS_PaymentController extends Netresearch_OPS_Controller_Abstract
{
    ...
    public function placeformAction()
    {
        ...
        if ($lastIncrementId) {
            ...
            if ($order->getState() == Mage_Sales_Model_Order::STATE_NEW) {
                ...
                if ($order->getId()) {
                    $order->setState(
                        Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage::helper('ops')->__('Start Barclaycard processing')
                    );
                    $order->save();
                }
            }
    ...
    {
        $helper = Mage::helper('ops');
        $helper->log($helper->__("Incoming accepted Barclaycard Alias Feedback\n\nRequest Path: %s\nParams: %s\n", $this->getRequest()->getPathInfo(), serialize($this->getRequest()->getParams())
        ));
    ...
    public function exceptionAliasAction()
    {
        ...
        $helper = Mage::helper('ops');
        $helper->log($helper->__("Incoming exception Barclaycard Alias Feedback\n\nRequest Path: %s\nParams: %s\n", $this->getRequest()->getPathInfo(), serialize($params)
        ));
    ...
    public function generateHashAction()
    {
        ...
        // OGNC-3 use main store id for orders from backend, since the feedback from Barclaycard could not be parsed in magento backend
